...
from apps.users.serializers import UserManageSerializer, UserSerializer
...
class ImportCredentialsPdfView(APIView):
    def post(self, request):
        ...
        if not isinstance(accounts, list) or not accounts:
            return Response({"detail": "accounts must be a non-empty list"}, status=status.HTTP_400_BAD_REQUEST)
...
class UserManageViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if not is_partial:
            return Response({"detail": "Method not allowed. Use PATCH."}, status=status.HTTP_405_METHOD_NOT_ALLOWED)
        if request.user.role not in {UserRole.CURATOR, UserRole.ADMIN}:
            return Response({"detail": "Permission denied."}, status=status.HTTP_403_FORBIDDEN)
